...
├── config.py                 env-overridable knobs + FILLER_PROMPT + LAYOUT_PROMPT
├── pipeline.py               run_all — sequences Stage A through F
└── stages/
    ├── sync_clap.py          flash + audio onset + csv event detection
    ├── transcribe.py         mlx-whisper wrapper, content-hash cache
    ├── analyze_llm.py        Claude-CLI / MLX-server dispatcher, pydantic validation
    ├── dead_zone_detect.py   sweep-line intersection of freeze + silence intervals
    ├── unify_segments.py     Segment dataclass + apply_cuts/dead_zones/cursor_zooms
    ├── render.py             chunked filter_complex builder + ffmpeg invoker
    ├── polish.py             ffmpeg-normalize wrapper + optional deep-filter denoise
    └── cursor_zoom.py        Cap algorithm port (AGPL-3.0 — see note below)
...
docs/future-improvements.md   face-detection roadmap + feathered edges
```
...
## 10. Current state (as of 2026-04-17)

- **All 6 milestones done.** M0 / M0b / M1 / M2 / M3 / M4 / M5 / M6 all ✅.
- **208 fast tests + 11 slow integration tests** passing.
- **Real-world validated** on a 34.7-min stitched 3-session recording →
  27:58 polished output (see README long-clip measurements).
- **Repo pushed to origin/main**: latest commit was `4f99491` at the time
  this CLAUDE.md was written.
- **Open items** — all in `docs/future-improvements.md`:
  1. Face-aware PIP auto-detection (Apple Vision / mediapipe / InsightFace)
  2. Feathered circle edge (soft mask)
  3. Circle border / drop shadow
  4. Multi-speaker PIP (deferred — no use case yet)
